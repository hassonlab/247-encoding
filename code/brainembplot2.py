...
def main():
    ...
    results = [  # two interactions (usually first glove and then gpt2)
        # "results/figures/glove-nnt/0shot-dat-e50-all3_NYU_class_IFG-n_81.csv",
        "results/figures/glove-aligned-nnt/0shot-dat-ned-all3_NYU_class_IFG-n_81.csv",
        # "results/figures/gpt2-fold-nnt/0shot-dat-old-all3_NYU_class_IFG-n_81.csv",
        "results/figures/gpt2-fold-aligned-nnt/0shot-dat-d-2-all3_NYU_class_IFG-n_81.csv",
        "results/figures/glove-concat20/0shot-dat-0-6-all3_NYU_class_IFG-n_81.csv",
        # "results/figures/gpt2-fold/0shot-dat-old-all3_NYU_class_IFG-n_81.csv"
        "results/figures/gpt2-fold-aligned/0shot-dat-d-2-all3_NYU_class_IFG-n_81.csv",
    ]

    for ax, file in zip(np.ravel(axes), results):
        ...
        ax.fill_between(
            lags, means - sems, means + sems, alpha=0.1, color="blue"
        )
        ...
        means = df_nn.mean()
        sems = df_nn.sem()
        ax.plot(lags, means, color=col)
        ax.fill_between(lags, means - sems, means + sems, alpha=0.1, color=col)
        ...
        ax.fill_between(
            lags, means - sems, means + sems, alpha=0.1, color="black"
        )
    ...
    fig.savefig("results/figures/interaction20.png")
